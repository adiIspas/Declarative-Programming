...
-- Version 1
--isBloodOrange (Orange variety segments) | variety `elem` tr = True
--                                        | otherwise = False
--isBloodOrange _ = False

-- Version 2
isBloodOrange x = case x of Orange variety segments -> variety `elem` tr
                            _ -> False

-- Version 3
--isBloodOrange x@(Orange variety segments) = Pentru a acesa tot obiectul si elemente din el

-- 2.
-- Varianta 1
{-bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments [] = 0
bloodOrangeSegments (x@(Orange variety segments):xs) | isBloodOrange x = segments + bloodOrangeSegments xs
                                                     | otherwise = bloodOrangeSegments xs
bloodOrangeSegments (x@(Apple variety value):xs) = bloodOrangeSegments xs
-}
-- Varianta 2
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments [] = 0
bloodOrangeSegments (x:xs) = case x of Orange variety segments -> if isBloodOrange x then segments + bloodOrangeSegments xs else bloodOrangeSegments xs
                                       _ -> bloodOrangeSegments xs
-- 3.
worms :: [Fruit] -> Int
worms [] = 0
worms (x:xs) = case x of Apple variety value -> if value == True then 1 + worms xs else worms xs
                         _ -> worms xs
...
prop_taut1 p = or [tautology p, satisfiable (Not p)]

prop_taut2 :: Prop -> Bool
prop_taut2 p = or [(satisfiable p), (tautology (Not p))]
