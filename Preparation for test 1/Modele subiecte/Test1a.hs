...
charToElem ch = if isDigit ch
                 then (prime !! (digitToInt ch))
                else error "Eroare"
...
strToInt :: String -> Int
strToInt xs = helperStrToInt xs (round(10**2))

helperStrToInt :: String -> Int -> Int
helperStrToInt [] _ = 0
helperStrToInt (x:xs) n = digitToInt x * n + helperStrToInt xs (div n 10)
...
enc :: String -> String
enc xs = concat [charToElem ch | ch <- xs]
...
position = zip [0..] prime

elm :: String -> Int
elm e = if length auxElm > 0 then head auxElm else -1
         where auxElm = [p | (p,x) <- position, x == e]
